...
class SpanController () {
    ...
    @Synchronized
    // TODO where to get the spannamese from
        ...
        for (spanName in spans) {
            ...
            if (span.parentId == null) {
                ...
                for (attribute in span.attributes) {
                    if (attribute.equalsIgnoreCase("http.method")) {
                        method = String(attribute.value).uppercase(Locale.getDefault())
                    }
                }
    ...
    private fun proceedLocalComponentForRelation(spanId: Long?, relation: Relation) {
        if (spanId != null) {
    ...
    private fun updateComponents(endpoint: Endpoint): Boolean {
        val serviceName: String = endpoint.serviceName.toUpperCase()
        val hardwareName = getHardwareName(endpoint)
        val instanceName = getInstanceName(endpoint)
